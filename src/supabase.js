...
function setStoredSession(session) {
  if (session) {
    localStorage.setItem("sbd_session", JSON.stringify(session));
  } else {
...
async function supabaseFetch(path, options = {}) {
  if (!SUPABASE_KEY) {
    throw new Error("Missing REACT_APP_SUPABASE_KEY");
  }
...
export const supabase = {
  ...
  async select(table, query = "") {
    return supabaseFetch(`/rest/v1/${table}${query}`, { method: "GET", prefer: "" });
  },
  ...
  async upsert(table, rows, onConflict = "id") {
    return supabaseFetch(`/rest/v1/${table}?on_conflict=${onConflict}`, {
      method: "POST",
